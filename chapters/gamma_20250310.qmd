...
## Background
The Gamma distribution models positively skewed, non-negative data, making it the ideal distribution to model a variety of public health phenomena, such as:

- **Survival analysis**: The Gamma distribution is a common choice for frailty models in survival analysis (@balanTutorialFrailtyModels2020)

- **Compartmental models for disease transmission**: SEIR models can replace the typical Exponential distribution with a Gamma distribution for more realistic models of disease transmission (@lloydRealisticDistributionsInfectious2001), (@wearingAppropriateModelsManagement2005)

- **Gene expression variability** Gene expression variability is typically modeled using the Negative Binomial distribution, which can be written as a Gamma-Poisson mixture model. (@hafemeisterNormalizationVarianceStabilization2019), (@wangGeneExpressionDistribution2018)


### Case Study: Time Until the 3rd Patient Arrival

Consider an **outpatient clinic** where patients arrive as a Poisson process at $\lambda=5$ per hour (about one every 12 minutes).  
Let $T_3$ be the total time from opening until the **third** arrival—so all three nurses have patients.

Then $T_3\sim\mathrm{Gamma}(\alpha=3,\lambda=5)$, and:

- $\mathbb E[T_3]=\frac{3}{5}=0.6$ hr ($\approx36$ min),
- $\Pr(T_3>20\text{ min}) = 1-F_{\Gamma}(1/3;3,5)\approx0.26$,
- the hazard increases over time because $\alpha>1$.

This model helps **staffing and surge planning**, showing how quickly capacity becomes fully used after opening.


### Current Uses in Biostatistics and Public Health

- **Survival analysis:** Models waiting times with *increasing or decreasing* hazards (beyond exponential).  
- **Reliability/biomedical devices:** Time to failure in multi-component systems.  
- **Health operations:** Total service or queue time for multiple sequential tasks.  
- **Bayesian inference:** Conjugate prior for Poisson and Exponential rates.  
- **Epidemiology:** Time from exposure to onset when incubation involves multiple stages.

The Gamma is often replaced by **Weibull** or **log-normal** when data show stronger tail behavior, but it remains analytically convenient and interpretable.


## Example with Data: Time to the Third Arrival

Simulate 400 “days,” each observing the time until the **3rd** patient arrives.  
Estimate $(\alpha,\lambda)$ via **Method of Moments (MoM)** and **Maximum Likelihood (MLE)**, and compare.

```{r}
set.seed(20251103)

alpha_true  <- 3
lambda_true <- 5     # per hour
n_days <- 400

# Total time to 3rd arrival (sum of 3 exponential waits)
Talpha <- replicate(n_days, sum(rexp(alpha_true, rate = lambda_true)))

head(Talpha)
mean(Talpha); var(Talpha)
```
...
Demonstrate that a sum of independent exponential-distributed random variables follow a Gamma distribution.

**Solution**:

Let $X_i \sim \text{Exp}(\lambda)$ and $Y = \sum\limits_{i=1}^{n}X_i$
