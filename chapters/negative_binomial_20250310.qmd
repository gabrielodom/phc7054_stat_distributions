...
The **Negative Binomial distribution** extends the Binomial model to situations where the number of trials is not fixed in advance.  
Instead, trials continue until a fixed number of **successes** \(r\) occurs, and the distribution describes the number of **failures** before the final success.  

- **Connection to Binomial:** While the Binomial counts successes in a fixed number of trials, the Negative Binomial counts failures until reaching \(r\) successes.  
- **Connection to Geometric:** When \(r=1\), the NB reduces to the Geometric distribution.  
- **Key property:** It naturally accounts for **overdispersion**, i.e., variance greater than the mean, making it more flexible than the Poisson distribution for real-world count data.  

The NB distribution is widely applied in **biostatistics, epidemiology, and public health** to model count outcomes such as number of hospital visits, infectious disease cases, or equipment failures, especially when variability exceeds Poisson expectations.


### Case Study: Negative Binomial Distribution in Modern Policy

A timely application of the Negative Binomial distribution is in **modeling COVID-19 case counts across counties**.  
Daily new cases often display **overdispersion**: some counties report very few cases, while others have sudden spikes. The Poisson distribution, which assumes equal mean and variance, underestimates this variability.  

For example, suppose Miami-Dade County reported the following daily new cases over a week:  
\{12, 14, 8, 20, 36, 10, 18\}.  

- The sample mean is \(\bar{x} = 17.4\).  
- The sample variance is \(s^2 = 85.3\), far larger than the mean.  

This suggests **overdispersion**, where a Negative Binomial model is more appropriate.  

Policy relevance:  
- **Public Health Planning:** Accurately modeling variation in daily case counts helps local authorities allocate testing resources [@lloydsmith2005].  
- **Hospital Preparedness:** By allowing for higher-than-expected peaks, NB models give more realistic forecasts of ICU demand [@hilbe2011].  
- **Equity in Resource Allocation:** Overdispersed models highlight counties at higher risk of extreme outbreaks, guiding targeted interventions [@pei2020].  

Thus, the Negative Binomial distribution provides a **robust framework** for decision-making when variability in outcomes is higher than expected under simple Poisson assumptions.


### Current Uses in Biostatistics and Public Health

The Negative Binomial distribution is widely applied in **biostatistics, epidemiology, and health policy**, particularly when data exhibit **overdispersion** (variance larger than the mean). Some common uses include:

- **Modeling disease incidence:** Overdispersed counts of infectious disease cases, such as COVID-19 or influenza, are often better described by the Negative Binomial than the Poisson [@lloydsmith2005; @pei2020].  
- **Health services research:** Counts of hospital visits, readmissions, or length of stay often display greater variability than expected under Poisson assumptions. NB regression models provide more reliable inference in these cases [@hilbe2011].  
- **Genomics and transcriptomics:** RNA-seq read counts are typically modeled with Negative Binomial distributions, forming the basis of popular tools like **DESeq2** and **edgeR** [@love2014; @robinson2010].  
- **Environmental epidemiology:** Studies of air pollution exposure and respiratory health often use Negative Binomial regression to model the number of adverse events per subject [@fuentes2010].  

Because of its flexibility, the Negative Binomial is considered a **standard alternative to the Poisson** in applied health research when variance substantially exceeds the mean.
...
As we can see, in order to achieve 5 wins total in each experiment, we needed 5 + `r NBk_int` "games" in those experiments. Now that we have the MGF and a sample of data, we can:
...
### Overdispersion in the Negative Binomial Distribution

The Negative Binomial distribution with parameters \(r > 0\) and \(0 < p < 1\) has mean and variance:  
...
1. Show that \(\text{Var}(X) > \mathbb{E}[X]\) whenever \(0 < p < 1\).  
2. Compute \(\text{Var}(X) - \mathbb{E}[X]\) and interpret the result in terms of “extra variance” compared to the Poisson distribution.  
3. Suppose \(r = 5\) and \(p = 0.7\). Calculate the mean and variance explicitly. How much larger is the variance than the mean?  
...
Count data can be modeled with the **Poisson** distribution.

$$
P(Y = y |\lambda) = \frac{e^{-\lambda}\lambda^{y}}{y!}, \ \ y = 0,1,2,\ldots
$$

In the Poisson distribution, the mean and variance are equal $E[X] = Var(X) = \lambda$.

However there are situations in real world data in which the variance is greater than the mean, $Var(X) > E[X]$ -- we say the data is **overdispersed**. To account for this, we can assume that $\lambda$ follows a distribution instead of being fixed.


### The Poisson-Gamma Mixture Model 

Assume $Y \sim Poisson(\lambda)$. Instead of a fixed $\lambda$, assume $\lambda \sim Gamma(\alpha, \beta)$. 
...
P(Y = y |\lambda) = \frac{e^{-\lambda}\lambda^{y}}{y!}, \ \ x = 0,1,2,\ldots \\

f(\lambda) = \frac{\beta^\alpha}{\Gamma(\alpha)}\lambda^{\alpha-1}e^{-\lambda\beta}, \lambda > 0 \\

\end{split}


### The Unconditional Probability $P(Y = y)$

Now if we want the $P(Y = y)$, we marginalize out $\lambda$:

\begin{split}

P(Y = y) &= \int_{0}^{\infty}P(Y=y|\lambda)f(\lambda)d\lambda \\

&= \int_{0}^{\infty}\frac{e^{-\lambda}\lambda^{y}}{y!}\frac{\beta^\alpha}{\Gamma(\alpha)}\lambda^{\alpha-1}e^{-\lambda\beta}d\lambda \\

&= \frac{\beta^\alpha}{y!\Gamma(\alpha)}\int_{0}^{\infty}e^{-\lambda}\lambda^y\lambda^{\alpha-1}e^{\lambda\beta}d\lambda \\

&= \frac{\beta^\alpha}{y!\Gamma(\alpha)}\int_{0}^{\infty}\lambda^{\alpha + y -1}e^{\lambda\beta-\lambda}d\lambda \\

&= \frac{\beta^\alpha}{y!\Gamma(\alpha)}\int_{0}^{\infty}\lambda^{\alpha + y -1}e^{-\lambda(\beta+1)}d\lambda  \ \ & &\text{kernel of } Gamma(\alpha+y,\beta+1)\\

&= \frac{\beta^\alpha}{y!\Gamma(\alpha)}\frac{\Gamma(\alpha+y)}{(\beta+1)^{\alpha+y}} \\

&= \frac{\beta^\alpha\Gamma(\alpha+y)}{(\beta+1)^{\alpha}(\beta+1)^yy!\Gamma(\alpha)} \\

&= \frac{\Gamma(\alpha+y)}{y!\Gamma(\alpha)}\left(\frac{\beta}{\beta+1}\right)^\alpha\left(\frac{1}{\beta+1}\right)^y & &\text{collect terms} \\

&= \frac{(a + y - 1)!}{y!(a-1)!}\left(\frac{\beta}{\beta+1}\right)^\alpha\left(\frac{1}{\beta+1}\right)^y & &\text{convert gamma to factorial} \\

&= \frac{(a + y - 1)!}{y!(a-1)!}\left(p\right)^\alpha\left(1-p\right)^y & & \text{let } p = \frac{\beta}{\beta+1} \\


&= {{\alpha+y-1}\choose{y}}\left(p\right)^\alpha\left(1-p\right)^y \sim NB(\alpha, p)

\end{split}

:::{.callout}
**The unconditional probability $P(Y=y)$ follows a negative binomial distribution, where, $\alpha$ is the number of successes, and $y$ is the number of failures.** 
:::


### How does this ensure $Var(Y) > E[Y]$?

This adds additional variability to $Y$ via the **law of total variance**:
...
&= \mu + \frac{\mu^2}{\alpha} \ \, \text{where } \mu = \alpha / \beta

\end{split}

Compare this variance $Var(Y) = E[\lambda] + Var(\lambda)$ to the original Poisson variable $Var(Y|\lambda) = \lambda$. We've inflated the variance of $Y$ by treating $\lambda$ as random.


In this parametrization, the variance of $Y$ is quadratic in $\mu$ at a rate controlled by the **dispersion parameter** $\alpha$, which is also the number of "successes" in the negative binomial distribution.
...
&= \frac{\alpha}{\frac{p}{1-p}} \text{, } \ \ \beta = \frac{p}{1-p} \\

&= \alpha\left(\frac{1-p}{p}\right)

\end{split}

Which is the typical form of the $E[X]$ for $X \sim NB(\alpha,p)$
...
Counts of daily **emergency department (ED) visits** often show variability greater than expected under a Poisson model (variance ≫ mean). A **Negative Binomial** model accommodates this overdispersion and typically yields better fit and uncertainty quantification than Poisson regression [@hilbe2011].

We’ll simulate 90 days of ED visit counts for two hospitals (A and B). Hospital B has a higher baseline and slightly greater day-to-day variability. We then compare Poisson vs. Negative Binomial fits.

```{r}
#| label: nb-example-simulate
#| message: false
#| warning: false
...
df <- tibble(
  day  = 1:n_days,
  # Hospital assignment alternating by week (just to make patterns visible)
  hosp = if_else(((day - 1) %/% 7) %% 2 == 0, "A", "B")
) |>
  mutate(
    mu   = if_else(hosp == "A", 12, 18),   # mean level
    size = if_else(hosp == "A", 6, 4),     # NB 'size' (smaller -> more overdispersion)
    # Simulate NB counts with mean/size parameterization
    y    = rnbinom(n = n(), mu = mu, size = size)
  )

# Quick diagnostics: mean vs variance by hospital
df_sum <- df |>
  group_by(hosp) |>
  summarise(
    mean_y = mean(y),
...
```
The mean–variance check helps flag overdispersion:

```{r}
#| label: nb-example-meanvar-plot

ggplot(df, aes(x = hosp, y = y)) +
  geom_boxplot(outlier.alpha = 0.5) +
  labs(x = "Hospital", y = "Daily ED visits",
       title = "Daily ED visits by hospital (simulated)")
```


### Fit Poisson vs. Negative Binomial

We model daily counts with a hospital indicator.  

- A **Poisson GLM** assumes  
$[\mathrm{Var}(Y \mid X) = \mathbb{E}(Y \mid X),]$  

- A **Negative Binomial GLM** relaxes this assumption and allows  
$[\mathrm{Var}(Y \mid X) > \mathbb{E}(Y \mid X),]$
through an extra **dispersion parameter** [@hilbe2011].


```{r}
#| label: nb-example-fit

# Poisson GLM
fit_pois <- glm(y ~ hosp, data = df, family = poisson())

# Negative Binomial GLM
fit_nb   <- glm.nb(y ~ hosp, data = df)
...
A lower AIC indicates better fit. Overdispersion can also be checked for the Poisson model via a simple dispersion statistic:

```{r}
#| label: nb-example-dispersion

# Residual deviance / df for Poisson (values >> 1 suggest overdispersion)
dispersion_pois <- sum(residuals(fit_pois, type = "pearson")^2) / fit_pois$df.residual
dispersion_pois
```
**Interpretation**

The Negative Binomial model should produce a lower AIC and a reasonable dispersion, reflecting extra-Poisson variability.

The coefficient for hospB (on the log scale) estimates the multiplicative difference in mean daily visits for B vs. A. Exponentiating the coefficient gives an incidence rate ratio (IRR):

```{r}
#| label: nb-example-irr

coef_nb <- coef(summary(fit_nb))
IRR     <- exp(coef_nb["hospB", "Estimate"])
IRR

```

An $IRR > 1$ indicates Hospital B’s expected daily visits exceed Hospital A’s, after accounting for overdispersion.

**Takeaway:** When count data show variance > mean, the Negative Binomial is a principled alternative to Poisson, improving fit and inference for policy-relevant outcomes like hospital demand [@hilbe2011].
